...
size_t	get_margin(t_select *g_slct)
{
	size_t	size;
	t_lst	*current;
...
void	draw_list(t_select *g_slct)
{
	t_lst	*list;
	int		style;
	int		x;
	...
	while (list)
	{
		...
		style = (list == g_slct->current) ? style | UNDERLINE : style;
		putstr_flag(list->label, style);
		list = list->next;
		x = (y < ft_get_winsz()->ws_row - 1) ? x : x + (int)g_slct->margin;
